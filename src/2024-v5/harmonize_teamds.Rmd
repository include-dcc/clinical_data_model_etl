...
Just `dstype` for now, until Trish's other annotations are manually reviewed
...
```{r}
teamds_conditions_harmonized <- teamds_participant %>%
  select(participantExternalId, dstype) %>% 
  left_join(condition_lookup_teamds,
            by = c("dstype" = "source_column_value")) %>% 
...
```{r}
teamds_conditions_harmonized %>% 
  write_csv(here::here("output", "teamds_conditions_dstype_only_2024-04-19.csv"))
```
