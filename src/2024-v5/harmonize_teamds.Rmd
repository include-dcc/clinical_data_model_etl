...
## UPDATE CONDITIONS FOR V6

- don't have to extract structured cols
- add height, weight to Measures
- add Event ID = baseline


```{r}
condition_lookup_teamds <- condition_codes_v6 %>% 
  filter(study == "TEAM-DS") %>% 
  unnest(source_column_value) %>% 
  select(-Notes) %>% 
  distinct()

condition_cols <- condition_lookup_teamds %>% pull(source_column) %>% unique()
```


```{r}
teamds_conditions <- teamds_clinical_data %>% 
  select(studyid, all_of(condition_cols)) %>% 
  ...
  filter(!is.na(study)) %>% 
  mutate(studyCode = study,
         participantExternalId = as.character(studyid),
...
Measures
Filter out participants where height or weight = "missing"

```{r}
measures_lookup_teamds <- measure_codes_v6 %>% 
  filter(study == "TEAM-DS")

measure_cols <- measures_lookup_teamds %>% pull(source_column) %>% unique()
```


```{r}
teamds_measures <- teamds_clinical_data %>% 
  select(studyid, age, all_of(measure_cols)) %>% 
  filter(height != "missing" & weight != "missing") %>% 
  mutate(age = round(365.25 * as.numeric(age), 0),
         bmi = as.character(round(as.numeric(bmi), 1))) %>% 
  #keep values as <chr> to preserve original decimal places
  pivot_longer(cols = c(-studyid, -age),
               names_to = "source_column",
               values_to = "measureValue") %>% 
  left_join(measures_lookup_teamds, by = "source_column") %>% 
  mutate(studyCode = study,
         participantExternalId = as.character(studyid),
         participantGlobalId = "TBD",
         eventId = "Baseline",
...
```{r}
teamds_condition_harmonized <- bind_rows(teamds_measures, teamds_conditions) %>%
  arrange(`Participant External ID`) %>% 
  mutate(across(everything(), ~replace_na(.x, ""))) 
```

Add Global IDs from Participant

```{r}
teamds_participant <- read_csv(synGet("syn55843095")$path) %>% 
  mutate(`Participant External ID` = as.character(`Participant External ID`))

teamds_condition_harmonized %>%
  select(-`Participant Global ID`) %>% 
  left_join(teamds_participant %>% 
              select(`Participant External ID`, `Participant Global ID`),
            join_by(`Participant External ID`)) %>% 
  relocate(`Participant Global ID`, .before = `Participant External ID`) %>% 
  write_csv(here::here("output", "teamds_conditions_harmonized_2024-12-18.csv"))
``` 
