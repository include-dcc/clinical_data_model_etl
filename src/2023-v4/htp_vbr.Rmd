...
#Next time also replace -1 Volume with NA
```{r}
...
```{r}
v3_datafiles_final <- v3_datafiles %>% 
...
## X01 FILES - minimal clinical data & biospecimen

numbers from Matt:
WGS from "PAXgene Whole Blood DNA" = 18 samples
WGS from "White Blood Cells DNA" = 1 sample
RNAseq from "PAXgene Whole Blood RNA" = 139 samples
RNAseq from "White Blood Cells RNA" = 416 samples

Minimal data fields for CHOP BIX:
race
gender
ethnicity
affected status
relationships
source text tissue type
age at event days
source text tumor descriptor
composition 
analyte type

```{r}
htp_2021_x01_minimal <- vbr_biospecimens_harmonized_20231005 %>% 
  filter(containerExternalId %in% vbr_x01_subset$ContainerID) %>% 
  select(studyCode, participantExternalId, participantGlobalId, 
         sampleExternalId, sampleGlobalId, sampleType, ageAtBiospecimenCollection,
         containerExternalId, containerGlobalId) %>% 
  left_join(vbr_x01_subset %>% 
              select(ContainerID, SAMPLE_ALIAS, SM_ID),
            by = c("containerExternalId" = "ContainerID")) %>% 
  left_join(vbr_participants_harmonized_final %>%
              select(`Participant External ID`, Sex, Race, Ethnicity,
                     `Down Syndrome Status`, `Family ID`, `Family Relationship`, `Family Type`),
            by = c("participantExternalId" = "Participant External ID")) %>% 
  mutate(source_text_tissue_type = "Normal",
         source_text_tumor_descriptor = NA,
         relationships = case_when(
           `Family Relationship` == "Sibling" & Sex == "Male" ~ "Brother",
           `Family Relationship` == "Sibling" & Sex == "Female" ~ "Sister",
           `Family Relationship` == "Parent" & Sex == "Male" ~ "Father",
           `Family Relationship` == "Parent" & Sex == "Female" ~ "Mother",
           TRUE ~ `Family Relationship`),
         analyte_type = case_when(
           str_detect(sampleType, "RNA") ~ "RNA",
           str_detect(sampleType, "DNA") ~ "DNA")
  ) %>% 
  select(studyCode, participantExternalId, participantGlobalId, 
         race = Race,
         gender = Sex, 
         ethnicity = Ethnicity, 
         affected_status = `Down Syndrome Status`,
         `Family ID`, 
         relationships, 
         sampleExternalId, sampleGlobalId, containerExternalId, containerGlobalId, SAMPLE_ALIAS, SM_ID,
         source_text_tissue_type, 
         age_at_event_days = ageAtBiospecimenCollection,
         source_text_tumor_descriptor,
         composition = sampleType,
         analyte_type)

htp_2021_x01_minimal %>% 
  write_csv(here::here("output", "htp_2021_x01_minimal_with_Broad_ids.csv"))

```

Check "Other" relationships - are these from multi-member families, or singletons?

```{r}
control_other_families <- htp_2021_x01_minimal %>%  
  filter(relationships == "Other") %>% 
  pull(`Family ID`) %>% 
  unique()

vbr_participants_harmonized_final %>%
  filter(`Family ID` %in% control_other_families) %>% 
  select(`Participant External ID`, `Family ID`, `Family Type`, `Father ID`, `Mother ID`,
         `Sibling ID`, `Other Family Member ID`, `Family Relationship`, `Age at First Patient Engagement`, Sex) %>% 
  add_count(`Family ID`, name = "n_in_family") %>% 
  arrange(n_in_family, `Family ID`)

#  write_csv(here::here("output", "htp_control_families.csv"))


#47 control singletons
# 3 multi-families - Other is the "Proband"

#  write_csv(here::here("output", "htp_relationships_other.csv"))
...
Subject-Consent Mapping:
SUBJECT_ID
CONSENT
SEX

TODO: add subject overlap with 2018 X01 from Christina

```{r}
htp_crossover <- read_csv(here::here("data", "FV_X7NHH41E_FV_5C7WPPHE_merged_sc_ds_updated.csv")) %>% 
  filter(str_detect(SUBJECT_ID, "HTP"))

length(unique(htp_crossover$SUBJECT_ID)) #537

unique(htp_2021_x01_minimal$Sex)
#M, F

htp_dbgap_subject_consent 

htp_2021_x01_minimal %>% 
  select(SUBJECT_ID = participantExternalId, Sex) %>%
  distinct() %>% 
  mutate(CONSENT = 1,
         SEX = if_else(Sex == "Male", 1, 2)) %>% 
  select(SUBJECT_ID, CONSENT, SEX) %>% 
  left_join(htp_crossover %>% select(SUBJECT_ID_crossover = SUBJECT_ID),
            by = c("SUBJECT_ID" = "SUBJECT_ID_crossover"), keep = TRUE) %>% 
  arrange(SUBJECT_ID) %>% 
  mutate(overlap_with_phs002330 = case_when(
    !is.na(SUBJECT_ID_crossover) ~ "Yes",
    TRUE ~ "No"
  )) %>% 
  select(-SUBJECT_ID_crossover) %>% 
  write_csv(here::here("output", "htp_2021_x01_dbGaP_SCwithOverlap.csv"))

#total X01 participants = 511
#438 overlap with crossover
#73 don't overlap
```
...
```{r}
htp_2021_x01_minimal %>% 
  select(SUBJECT_ID = participantExternalId, SAMPLE_ID = sampleExternalId) %>% 
  distinct() %>% 
  write_csv(here::here("output", "htp_2021_x01_dbGaP_SSM.csv"))
...
**2023-11-16** check Christina's old vs new sample IDs

```{r}
christina_ids <- read_csv(here::here("data", "htp_new_sample_ID_mappings_for_DS.csv"))

matt_export <- read_delim(here::here("data", "HTP_DCC_VBR_BIOSPECIMENS_PL_corrected_3_DNATube_297A_2023-10-06.txt"), "\t")
```

```{r}
christina_ids %>% 
  left_join(matt_export %>% select(SampleID, SampleID_old),
            by = c("OLD_SAMPLE_ID" = "SampleID_old")) %>% 
  mutate(id_correct = if_else(NEW_SAMPLE_ID == SampleID, TRUE, FALSE)) %>% view()
```
two Old Sample IDs didn't have matches in Matt's export:

HTP0710A_DNA    HTP0710A_PAXgeneWholeBloodDNA
HTP0714A_DNA    HTP0714A_PAXgeneWholeBloodDNA


**2023-11-16** Convert -1 volume to `NA`, then convert to "" during write_csv
```{r}
vbr_biospecimens_harmonized_20231005_update_volume <- read_csv(synGet("syn52599895")$path)

vbr_biospecimens_harmonized_20231005_update_volume %>% 
  mutate(Volume = na_if(Volume, -1)) %>%
  write_csv(here::here("output", "vbr_biospecimens_harmonized_20231116.csv"),
            na = "")

```


Checking HTP biospecimen report downloaded from QA portal
Note that all Collections/Parents are also Samples in Matt's extract,
but in QA Collections are separate from Samples

```{r}
qa_biospecimens <- read_tsv(here::here("data", "include-biospecimen-table-2023-11-17.tsv"))
```

```{r}
# none of the Collection IDs are in Sample ID col
n_distinct(qa_biospecimens$`Collection ID`) #1307
length(setdiff(qa_biospecimens$`Collection ID`, qa_biospecimens$`Sample ID`)) #1307
qa_biospecimens %>% filter(`Sample ID` %in% qa_biospecimens$`Collection ID`) #0

# all Parent Samples are in Sample ID UNLESS they are also Collections
n_distinct(qa_biospecimens$`Parent Sample ID`) #4399
length(setdiff(qa_biospecimens$`Parent Sample ID`, qa_biospecimens$`Sample ID`)) #1307
length(setdiff(qa_biospecimens$`Parent Sample ID`, qa_biospecimens$`Collection ID`)) #3092
qa_biospecimens %>% filter(`Sample ID` %in% qa_biospecimens$`Parent Sample ID`)

n_distinct(qa_biospecimens$`Sample ID`) #12233
```

```{r}
# Unique Collection & Sample IDs from QA
qa_samples_collections <- unique(c(qa_biospecimens$`Collection ID`, qa_biospecimens$`Sample ID`))
length(qa_samples_collections) #13540

# All of these are in harmonized data
vbr_biospecimens_harmonized_20231005 %>% 
  filter(sampleGlobalId %in% qa_samples_collections) %>% 
  pull(sampleGlobalId) %>% n_distinct()
#13540

# these are not
harmonized_samples_not_in_qa <- vbr_biospecimens_harmonized_20231005 %>% 
  filter(!(sampleGlobalId %in% qa_samples_collections))
#pull(sampleGlobalId) %>% n_distinct() #975

```


```{r}
unique(harmonized_samples_not_in_qa$sampleType)
```

All missing collections have containers
```{r}
harmonized_samples_not_in_qa %>% 
  filter(is.na(containerExternalId))
```

975 unique Sample Global IDs
```{r}
length(unique(harmonized_samples_not_in_qa$sampleGlobalId))
```
...
```{r}
harmonized_samples_not_in_qa %>% 
  write_csv(here::here("output", "harmonized_samples_collections_not_in_qa.csv"))
```


## 2023-12-08 More QA on Global IDs from Christina

Christina can't find global IDs for a bunch of samples, including the 12 left accidently
left out of KF-CHDALL

There are "Tube" descriptors in dewrangle but not the non-Tube
Possible that I forgot to replace the DNA_Paxgene descriptors?
Check my harmonized data and Matt's export to make sure all SAmpleID_old are DNA-PAxgene
```{r}
htp_df_manifest_no_sample_globals <- read_csv(
  here::here("data", "htp_df_manifest_no_sample_globals.csv")) %>% 
  pull(`Sample External ID`)
```

All in Christina's list are in the final harmonized VBR biospecimens
```{r}
vbr_biospecimens_harmonized_20231005 %>% 
  filter(sampleExternalId %in% htp_df_manifest_no_sample_globals) %>% 
  pull(sampleExternalId) %>% unique() %>% length()
```

```{r}
matt_latest_export <- read_tsv(synGet("syn52138216")$path)

dewrangle_20231203 <- read_csv(here::here("data", "global-descriptors-htp-all.csv"))

```

```{r}
matt_latest_export %>% 
  filter(SampleID %in% htp_df_manifest_no_sample_globals) %>% 
  select(SampleID, SampleID_old, Sample_global_id) %>% 
  filter(!is.na(SampleID_old)) %>% 
  distinct() %>% 
  left_join(dewrangle_20231203 %>% select(descriptor, global_id),
            by = c("Sample_global_id" = "global_id"))

#425, because this is "all" globals and there's an old & new descriptor for HTP0297A6_DNA
#%>% filter(str_detect(descriptor, "297A"))

#otherwise everything matches -> somehow I missed updating these descriptors?
```

Dewrangle input:

Can't figure out how I missed updating these descriptors,
but will go ahead and update now (filter out SampleID_old = 297A6_DNA)
...
matt_latest_export %>% 
  ...
  left_join(dewrangle_20231203 %>% select(descriptor, global_id),
            by = c("Sample_global_id" = "global_id")) %>% 
  filter(SampleID_old != "HTP0297A6_DNA") %>% 
  select(-descriptor, -SampleID_old, 
         descriptor = SampleID, global_id = Sample_global_id) %>% 
  mutate(fhir_resource_type = "Specimen", .after = descriptor) %>% 
  write_csv(here::here("output", "dewrangle_input_2023-12-08.csv"))
```
Download "all" descriptors
```{r}
dewrangle_20231208 <- read_csv(here::here("data", 
                                          "global-descriptors-htp-all-20231208.csv"))

dewrangle_20231208 %>% 
  filter(descriptor %in% htp_df_manifest_no_sample_globals)
```

## 2023-12-13 "Old" Sample External IDs showed up in Christina's X01 datafile manifest
Looks like i forgot (or decided not to for some reason) update descriptors in dewrangle

Get Christina's manifest and dewrangle with NEWEST (not all) descriptors
```{r}
christina_htp_x01_datafiles <- read_csv(synGet("syn53137966")$path)

dewrangle_newest_20231214 <- read_csv(here::here("data", 
                                                 "global-descriptors-htp-newest-20231214.csv"))

```

Make sure all descriptors are in dewrangle:
```{r}
christina_htp_x01_datafiles %>% 
  select(`Sample Global ID`, `Sample External ID`) %>% 
  left_join(dewrangle_newest_20231214 %>% select(global_id, descriptor),
            by = c(`Sample Global ID` = "global_id")) %>% 
  filter(is.na(descriptor)) #0

```

Figure out which are "old" IDs - from matt's export
make this into dewrangle input
```{r}
x01_descriptor_changelog <- christina_htp_x01_datafiles %>% 
  select(`Sample Global ID`, `Sample External ID`) %>% 
  left_join(matt_latest_export %>% select(SampleID, SampleID_old, Sample_global_id) %>% distinct(),
            by = c(`Sample Global ID` = "Sample_global_id")) %>% 
  filter(`Sample External ID` != SampleID) %>% #365
  #filter(`Sample External ID` != SampleID_old) #0
  select(`Sample Global ID`, 
         old_descriptor = `Sample External ID`,
         new_descriptor = SampleID) %>% 
  distinct() 


x01_descriptor_changelog %>% write_csv(here::here("output",
                                                  "x01_descriptor_changelog.csv"))
```


```{r}
x01_descriptor_changelog %>% 
  select(descriptor = new_descriptor, global_id = `Sample Global ID`) %>% 
  mutate(fhir_resource_type = "Specimen", .after = descriptor) %>% 
  write_csv(here::here("output", "dewrangle_input_20231214.csv"))
```
