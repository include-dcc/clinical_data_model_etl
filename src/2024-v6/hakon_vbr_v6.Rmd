...
```{r}
hakon_vbr <- read_xlsx(synGet("syn63100119")$path, sheet = "CAG_ALL DATA")
...
```{r}
n_distinct(hakon_vbr$`Sample External ID`) #6768
n_distinct(hakon_vbr$`Parent Sample External ID`) #2871
n_distinct(hakon_vbr$`Participant External ID`) #1152
n_distinct(hakon_clinical_all$`Participant ID`) #1152
n_distinct(hakon_vbr$`ALQ ID`) #6026
...
hakon_vbr %>% 
  ...
  pull(`Parent Sample External ID`) %>% n_distinct() #1670

# Sample ID-ALQ ID pairs
hakon_vbr %>% get_dupes(`Sample External ID`, `ALQ ID`) #0
hakon_vbr %>% get_dupes(`Parent Sample External ID`, `ALQ ID`) #0
...
```{r}
hakon_vbr %>% 
  select(`Collection Sample Type`, `Parent Sample Type`, `Sample Type`, `Laboratory Procedure`) %>% 
  unique()
```
