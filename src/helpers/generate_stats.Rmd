...
harmonized_participant <- read_csv(synGet("syn55998061")$path)
harmonized_condition <- read_csv(synGet("syn56666593")$path)
```
...
```{r}
harmonized_biospecimen <- read_csv(synGet("syn52599895")$path)

biospecimen_factor_cols <- c("Sample Type", "Parent Sample Type", 
"Collection Sample Type", "Laboratory Procedure", "Biospecimen Storage", "Sample Availability", "Container Availability")
```


```{r}
length(unique(harmonized_biospecimen$`Container External ID`))
```

```{r}
harmonized_biospecimen %>% 
  #filter(`Study Code` == studycode) %>% 
  mutate(across(all_of(biospecimen_factor_cols), ~as.factor(.x))) %>% 
  select(`Participant External ID`, all_of(biospecimen_factor_cols)) %>% 
  pivot_longer(cols = -`Participant External ID`,
               names_to = "variable",
               values_to = "value") %>% 
  count(variable, value) %>% 
...
Unique Family IDs
```{r}
#read_csv(synGet("syn52399152")$path) %>% 
harmonized_participant %>% 
  filter(`Study Code` == "X01-Hakonarson") %>% 
  filter(!is.na(`Family ID`)) %>% 
  pull(`Family ID`) %>% n_distinct()
```
