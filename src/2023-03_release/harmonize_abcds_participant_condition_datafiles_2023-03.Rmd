...
```{r}
has_siblings <- abcds_control_linkage %>% pull(ABCDS_ID_DS)

abcds_demo_BL_harmonized <- abcds_demo_BL %>%  
  left_join(abcds_control_linkage %>% select(ABCDS_ID_SC, ABCDS_ID_DS),
  ...
  mutate(familyId = case_when(
    ...
    subject_label %in% has_siblings ~ subject_label
  ), .after = subject_label) %>% 
