...
hakon_aliquot_global_ids <- read_csv(here::here("output", "global-descriptors-X01-Hakon.csv")) %>% 
  ...
  mutate(descriptor = as.double(descriptor))
...
```{r}
write_csv(hakon_biospecimen, here::here("output", "hakon_biospecimen_TEST.csv"))
```
...
```{r}
hakon_biospecimen <- read_csv(here::here("output", "hakon_biospecimen_TEST.csv"))
...
hakon_biospecimen <- hakon_biospecimen %>% 
  ...
  mutate(`Sample Global ID` = global_id,
         `Parent Sample Type` = case_when(
           `Parent Sample Type` == "PBMC" ~ "PBMCs",
           `Parent Sample Type` == "Blood" ~ "Peripheral Whole Blood",
           `Parent Sample Type` == "Saliva" ~ "Saliva"),
         `Collection Sample Type` = case_when(
           `Collection Sample Type` == "Blood Draw" ~ "Peripheral Whole Blood",
           `Collection Sample Type` == "Saliva Kit" ~ "Saliva"
         )) %>% 
  select(-global_id)
