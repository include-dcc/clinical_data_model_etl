...
```{r}
n_distinct(hakon_vbr$`Sample ID (Calc)`) #4151
n_distinct(hakon_vbr$`Container ID`) #10855
n_distinct(hakon_vbr$`Parent Sample ID (Calc)`) #4473
n_distinct(hakon_vbr$`Parent Sample Container ID`) #4473
...
hakon_datafiles_v4 <- hakon_datafiles_v3 %>% 
  left_join(hakon_biospecimen_draft %>% select(`Sample External ID`, `Sample Global ID`, `Container External ID`, `Container Global ID`),
            by = "Sample External ID")

hakon_datafiles_v4 %>%
  left_join(hakon_vbr %>% select(`Sample ID (Calc)`, `Container ID`),
            by = c("Container External ID" = "Container ID"))

##TODO: add "KFDS" to Container IDs
```
