...
#participants that don't have global IDs yet
...
setdiff(biospecimen_need_new_participant_id, htp_dewrangle_patients$descriptor)
# these are already in dewrangle! does it just need an update?
# maybe don't use the comments - just check which samples already have Participant Global ID
# and use those to populate the other container rows from those samples

htp_dewrangle_patients %>% filter(descriptor %in% biospecimen_need_new_participant_id)
vbr_biospecimens %>% filter(ParticipantExternalID %in% biospecimen_need_new_participant_id) %>% 
  select(ParticipantExternalID, Participant_global_id, SampleID, SampleID_old, Sample_global_id, HTP_comment) %>% 
  distinct() %>% 
  arrange(ParticipantExternalID) %>% view()
# I think some of the NEW samples just need to be associated with Participant Global ID, not new global id
# The ones where comment = needs new global ID might not actually?
# also in here are samples where SampleID != SampleID_old but comment = "exact match"
...
all_participants_needing_new_globals <- unique(c(participants_needing_global_ids, biospecimen_need_new_participant_id))
#274

# compare to existing dewrangle
setdiff(htp_dewrangle_patients$descriptor, all_participants_needing_new_globals)
setdiff(all_participants_needing_new_globals, htp_dewrangle_patients$descriptor)
```


Get global IDs
```{r}
tibble(descriptor = all_participants_needing_new_globals,
